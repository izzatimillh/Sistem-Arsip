      ...
      <section class="col-lg-12">
        ...
        <div class="box box-info">
          <div class="box-header">
           <?php 
           $sss = $_SESSION['section'];
           $ssss = mysqli_query($koneksi,"SELECT * FROM section WHERE section_id='$sss'");
           $sssss = mysqli_fetch_assoc($ssss);
           ?>
           <h3 class="box-title">Laporan Stock (<b><?php echo $sssss['section_desc'] ?></b>)</h3>
                  ...
                  <form action="stock_import.php" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                      ...
                      <select class="form-control" name="section" required="required">
                        <?php 
                        $sss = $_SESSION['section'];
                        $section = mysqli_query($koneksi,"SELECT * FROM section WHERE section_id='$sss'");
                        while($s = mysqli_fetch_array($section)){
                          ?>
                          <option <?php if($_SESSION['section'] == $s['section_id']){ echo "selected='selected'"; } ?> value="<?php echo $s['section_id'] ?>"><?php echo $s['section_desc'] ?></option>
                          <?php 
                        }
                        ?>
                      </select>
        ...
        <div class="box-body">
         ...
         <div class="col-lg-4 col-lg-offset-2">
          ...
          if(isset($_GET['tanggal'])){
            ...
            $tanggal_fix = date('Y-m-d',strtotime($tanggal_fix));
            $stock = mysqli_query($koneksi,"SELECT SUM(stock_qty_theory) as total_theory, SUM(stock_qty_actual) as total_actual, SUM(stock_qty_variant) as total_variant FROM stock,section WHERE stock_section=section_id AND stock_section='$s' AND DATE(stock_date)='$tanggal_fix' ORDER BY stock_id DESC");
          }else{
            $hari_ini = date('Y-m-d');
            $stock = mysqli_query($koneksi,"SELECT SUM(stock_qty_theory) as total_theory, SUM(stock_qty_actual) as total_actual, SUM(stock_qty_variant) as total_variant FROM stock,section WHERE stock_section=section_id AND stock_section='$s' AND DATE(stock_date)='$hari_ini' ORDER BY stock_id DESC");
          }
        ...
        <table class="table table-bordered table-striped" id="table-datatable">
          ...
          <tbody>
            ...
            if(isset($_GET['tanggal'])){
              ...
              $tanggal_fix = $t[2].'-'.$t[1].'-'.$t[0];
              $tanggal_fix = date('Y-m-d',strtotime($tanggal_fix));
              $stock = mysqli_query($koneksi,"SELECT * FROM stock,section WHERE stock_section=section_id AND stock_section='$s' AND DATE(stock_date)='$tanggal_fix' ORDER BY stock_id DESC");
            }else{
              $hari_ini = date('Y-m-d');
              $stock = mysqli_query($koneksi,"SELECT * FROM stock,section WHERE stock_section=section_id AND stock_section='$s' AND DATE(stock_date)='$hari_ini' ORDER BY stock_id DESC");
            }
            while($s = mysqli_fetch_array($stock)){
              ...
              <tr>
                ...
                <td>
                  <!-- <a class="btn btn-sm btn-warning" href="stock_edit.php?id=<?php echo $s['stock_id']; ?>"><i class="fa fa-wrench"></i> QTY</a> -->
                  <!-- <a  href="stock_edit.php?id=<?php echo $s['stock_id']; ?>"></a> -->

                  <button type="button" class="btn btn-warning btn-sm" data-toggle="modal" data-target="#lap_stock_<?php echo $s['stock_id']; ?>"><i class="fa fa-wrench"></i> QTY</button>
                  <!-- Modal -->
                  <div id="lap_stock_<?php echo $s['stock_id']; ?>" class="modal fade" role="dialog">
                    <div class="modal-dialog">
                      <div class="modal-content">
                        <div class="modal-header">
                          <button type="button" class="close" data-dismiss="modal">&times;</button>
                        ...
                        <div class="modal-body">
                          <p>Update Qty Actual.</p>
                          <form action="stock_update2.php" method="post">
                            <div class="form-group">
                              <label>Section</label>
                              <input type="hidden" name="id" value="<?php echo $s['stock_id']; ?>" class="form-control">
                              <input type="number" name="qty_actual" value="<?php echo $s['stock_qty_actual']; ?>" class="form-control">
                            </div>
                            <br/>
                            <br/>
